...
#define CHECK_EQUAL(actual, expected) {                \
   for (auto&& [a, e] : view::zip(actual, expected)) { \
      CHECK(a == e);                                   \
   }                                                   \
}                                                      \
...
std::vector<ltcpp::token> generate_tokens(std::string const& source, ltcpp::reporter& report)
{
   ...
   auto token_generator = view::generate([&in, &report, &cursor]{
      return ltcpp::generate_token(in, report, cursor);
   });
   for (auto&& token : token_generator) {
